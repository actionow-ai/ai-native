...
- 可浏览原型交接记录: [`docs/browsable-prototype-handoff.md`](docs/browsable-prototype-handoff.md)
...
如果 `8173` 上已经是同一个蝴蝶地球仪预览，脚本会复用并打印 URL；如果默认端口被其他服务占用，脚本会自动切换到空闲端口并打印实际 URL。需要固定端口时，运行 `npm start -- --port 8199`。
...
它默认使用 `8174`，避免和人工预览的 `8173` 互相复用；需要指定端口时可运行 `BUTTERFLY_GLOBE_PORT=8199 npm run test:browser`。
