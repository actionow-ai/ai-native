...
`preflight`、`candidates` 和 Wikidata 候选文件只用于加速策展，不是公开地图数据。`data/species-seed.partial.json` 只用于暂存已经过人工核查的 starter 记录；正式交付仍以至少 10 条记录的 `data/species-seed.json` 为准，且每条记录必须通过 `data/species-seed.schema.json` 的 `approved` 门槛，并由人工确认地点来源、坐标精度和图片授权。
