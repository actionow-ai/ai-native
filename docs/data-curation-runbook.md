...
`data/species-source-review.queue.json` 是核查工作队列，不是公开 seed 数据。每个候选先补齐 `evidence.typeLocality` 和 `evidence.imageLicense`，再移动到 `data/species-seed.partial.json`；只有通过 schema/source gate 的 partial 才能同步进正式 `data/species-seed.json`。
...
`data/species-seed.partial.json` 存放已经通过人工来源核查的 starter 记录，用来给正式 seed 数据提供字段和证据样例。当前首版 `data/species-seed.json` 已从 10 条通过 schema gate 的 partial 记录物化；后续扩充时仍先补 partial，再重新跑 gate 后同步正式 seed。
